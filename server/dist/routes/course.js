...
const fallbackCourses = [
    {
        ...
        description: "Learn the fundamentals of React.js including components, state management, and hooks. Perfect for beginners looking to start their frontend journey.",
    ...
    },
    {
        ...
        description: "Master backend development with Node.js and Express. Build scalable APIs, handle authentication, and work with databases.",
    ...
    },
    {
        ...
        description: "Dive into data science with Python. Learn pandas, numpy, matplotlib, and machine learning basics with real-world projects.",
    ...
    },
    {
        ...
        description: "Learn design principles, user research, wireframing, and prototyping. Create beautiful and functional user interfaces.",
    ...
    },
    {
        ...
        description: "Build and deploy machine learning models using TensorFlow. Cover neural networks, deep learning, and AI applications.",
    ...
    },
    {
        ...
        description: "Create cross-platform mobile apps with Flutter and Dart. Build beautiful, native-quality apps for iOS and Android.",
    ...
    },
    {
        ...
        description: "Master DevOps practices with Docker, Kubernetes, AWS, and CI/CD pipelines. Deploy and scale applications in the cloud.",
    ...
    },
    {
        ...
        description: "Understand blockchain technology and build decentralized applications (DApps) using Solidity and Web3.",
    ...
    },
    {
        ...
        description: "Master digital marketing strategies, SEO, social media marketing, and data analytics to grow online presence.",
...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        // Build query
        let query = course_model_1.default.find(filter)
...
router.post("/:id/progress", auth_middleware_1.protect, async (req, res) => {
    try {
...
router.get("/:id/progress", auth_middleware_1.protect, async (req, res) => {
    try {
...
router.get("/categories/list", async (req, res) => {
    try {
        const categories = await course_model_1.default.distinct("category", { status: "published" });
...
router.get("/stats/overview", async (req, res) => {
    try {
        const totalCourses = await course_model_1.default.countDocuments({ status: "published" });
        const totalStudents = await course_model_1.default.aggregate([
            { $match: { status: "published" } },
            { $project: { studentCount: { $size: "$studentIds" } } },
            { $group: { _id: null, total: { $sum: "$studentCount" } } },
        ]);
        const categoriesStats = await course_model_1.default.aggregate([
            { $match: { status: "published" } },
            { $group: { _id: "$category", count: { $sum: 1 } } },
            { $sort: { count: -1 } },
        ]);
        const levelStats = await course_model_1.default.aggregate([
            { $match: { status: "published" } },
...
router.post("/", auth_middleware_1.protect, auth_middleware_1.instructorOnly, async (req, res) => {
    try {
        const { title, description, duration, level, category, price, syllabus, prerequisites, learningOutcomes, totalLessons, estimatedHours, language, certificate, thumbnail, videoIntro, } = req.body;
        ...
        const populatedCourse = await course_model_1.default.findById(course._id).populate("instructor", "name email");
...
router.put("/:id", auth_middleware_1.protect, auth_middleware_1.instructorOnly, async (req, res) => {
    try {
        const course = await course_model_1.default.findById(req.params.id);
        if (!course) {
            return res.status(404).json({ message: "Course not found" });
        ...
        const updatedCourse = await course_model_1.default.findByIdAndUpdate(req.params.id, { ...req.body, updatedAt: new Date() }, { new: true, runValidators: true }).populate("instructor", "name email");
...
router.delete("/:id", auth_middleware_1.protect, auth_middleware_1.instructorOnly, async (req, res) => {
    try {
        const course = await course_model_1.default.findById(req.params.id);
        if (!course) {
            return res.status(404).json({ message: "Course not found" });
        ...
        await course_model_1.default.findByIdAndDelete(req.params.id);
...
router.post("/:id/enroll", auth_middleware_1.protect, async (req, res) => {
    try {
        const course = await course_model_1.default.findById(req.params.id);
        if (!course) {
            return res.status(404).json({ message: "Course not found" });
        ...
        await course.save();
        const updatedCourse = await course_model_1.default.findById(req.params.id)
            .populate("instructor", "name email")
            .populate("studentIds", "name email");
...
router.post("/:id/unenroll", auth_middleware_1.protect, async (req, res) => {
    try {
        const course = await course_model_1.default.findById(req.params.id);
        if (!course) {
            return res.status(404).json({ message: "Course not found" });
...
router.get("/:id/enrollment-status", auth_middleware_1.protect, async (req, res) => {
    try {
        const course = await course_model_1.default.findById(req.params.id);
        if (!course) {
            return res.status(404).json({ message: "Course not found" });
